...
public class MyController_yerin {
    ...
    public String mypage_wishlist(@RequestParam(value="page", defaultValue="1") String page,
            Model model, HttpServletRequest request) {
        String users_id = (String) request.getSession().getAttribute("users_id");
        List<ProductDto> wishlist = productService.wishList(users_id, page, num_page_size);
        int wishCount = productService.wishCount(users_id);
        int pageNum = (int) Math.ceil((double) wishCount / num_page_size);
    ...
    public String product01_1(@RequestParam("product_idx") int product_idx,
        ...
        List<Product_qnaDto> qnaList = qnaService.qnaList(standard, String.valueOf(product_idx), qnaPage,
                num_page_size);
        if (qnaPageNum == 0) {
            qnaPageNum = 1;
        }
...
//			model.addAttribute("orderList", orderList);
    ...
    HttpServletRequest request, Model model) {
        ...
        List<WishlistDto> wishlist = wishService.wishList(users_id);
        for(int i=0; i<wishlist.size(); i++) {
            if(wishlist.get(i).getList_product_idx() == product_idx) {
                return "이미 관심상품에 추가된 상품입니다.";
            }
        }
        
        dto.setList_users_id(users_id);
        dto.setList_product_idx(product_idx);
        dto.setList_count(list_count);
        wishService.insertWish(dto);
        
        return "상품을 관심상품에 추가했습니다.";
    }
